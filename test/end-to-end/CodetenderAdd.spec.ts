...
describe('codetender add', () => {
  ...
  it.each([true, false])('should handle existing files with overwrite %s', async (overwrite) => {
    ...
    await expect(fileContains(`output/test-add/${overwrite}/still-here.txt`, 'This existing file that does not match template should be unchanged so foo should still say foo.')).resolves.toEqual(true);
    if (overwrite) {
