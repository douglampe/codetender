...
describe('ConfigParser', () => {
  describe('readConfig()', () => {
    ...
    it('should throw error on major version mismatch', async () => {
      ...
      await expect(parser.readConfig('bar.json', true)).rejects.toEqual(
        new Error(`This version of codetender requires configuration schema version ${ct.state.config.schemaVersion}.`),
      );
    });
    ...
    it('should log warning on older template version', async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith(
        'Warning: This template specifies an older version of the codetender configuration schema (1.0.0). Some features may not be supported.',
      );
    });
    ...
    it('should log warning on newer template version', async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith(
        'Warning: This template requires a newer version of the codetender configuration schema (1.999.0). Some features may not be supported.',
      );
    });
    ...
    it('should throw error if remote dest is not valid', async () => {
      ...
      await expect(parser.readConfig('bar.json', true)).rejects.toEqual(
        new Error('Configuration Error: Remote destinations must be one level down from the root.'),
      );
    });
    ...
    it('should parse tokens and merge values', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce('{ "tokens": [{"pattern": "foo", "prompt": "Replace with"}, {"pattern": "baz", "replacement": "BAZ"}]}');
      ...
      expect(ct.state.process.tokens).toEqual([
        { pattern: 'foo', prompt: 'Replace with' },
    ...
    it('should parse remote', async () => {
      ...
      jest
        .spyOn(fs.promises, 'readFile')
        .mockResolvedValueOnce('{ "remote": [{"src": "foo", "dest": "/", "tokens": [{ "pattern": "one", "replacement": "two"}]}]}');
    ...
    it('should parse noReplace', async () => {
      ...
      expect(ct.state.process.noReplace).toEqual(['**/.git/', '.codetender', 'no-replace.js']);
    });
    ...
    it('should parse ignore', async () => {
      ...
      expect(ct.state.process.ignore).toEqual(['**/.git/', '.codetender', 'ignored.js']);
    });
